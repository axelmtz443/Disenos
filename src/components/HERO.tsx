...
const OrbitalSystem = () => {
  ...
  const differentiators = [
    { 
      title: 'Más y Mejores Ventas', 
      desc: 'Escalamos tus conversiones de forma constante.',
      icon: TrendingUp 
    },
    { 
      title: 'Optimización de Recursos', 
      desc: 'Menos desperdicio, mayor ROI en pauta.',
      icon: PieChart 
    },
    { 
      title: 'Crecimiento Sostenido', 
      desc: 'Estructuramos embudos sólidos a largo plazo.',
      icon: LineChart 
    },
    { 
      title: 'Planeación Estratégica', 
      desc: 'Campañas diseñadas en base a datos reales.',
      icon: Target 
    },
    { 
      title: 'Mayor Competitividad', 
      desc: 'Destaca y domina tu mercado vertical.',
      icon: Trophy 
    }
  ];

  // Bucle de animación puro a 60 FPS (sin lecturas del cursor)
  useEffect(() => {
    const animate = () => {
...
export default function App() {
  return (
    ...
    >
      ...
      {/* Contenedor Principal en Dos Columnas (Textos Izquierda | Sistema 3D Derecha) */}
      <div className="max-w-7xl mx-auto px-6 sm:px-10 lg:px-16 w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center flex-1 pt-12 lg:pt-20 pb-8 relative z-10">
        ...
        <div className="lg:col-span-5 flex flex-col items-start text-left space-y-6 lg:space-y-8 z-20">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 w-full sm:w-auto">
            <button className="px-7 py-3.5 rounded-xl bg-gradient-to-r from-[#009fe3] to-[#599ddf] text-white font-bold text-sm shadow-xl shadow-blue-500/10 hover:shadow-blue-500/20 hover:brightness-110 active:scale-95 transition-all flex items-center justify-center gap-2">
              <span>Contactar</span>
              <ArrowRight className="w-4 h-4" />
            </button>
            
            <button className="px-7 py-3.5 rounded-xl bg-zinc-950/40 border border-zinc-800/80 hover:bg-zinc-900/60 text-zinc-300 hover:text-white font-semibold text-sm active:scale-95 transition-all flex items-center justify-center gap-2">
        ...
        {/* Columna Derecha: El Sistema Orbital */}
      ...
      {/* Bloque Inferior de Mensaje de Marca (Centrado) */}
      <div className="w-full relative z-10 border-t border-zinc-900/60 bg-gradient-to-b from-transparent">
        {/* Se cambió max-w-4xl a max-w-7xl para expandir el contenedor principal */}
