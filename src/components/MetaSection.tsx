...
interface Ad {
  ...
  pageName: string;
  pageLogo: string;
  pageFallbackLogoText: string;
  mainText: string;
  type: 'single-video' | 'carousel' | 'single-image' | string;
  videoUrl?: string;
  videoPoster?: string;
...
const ADS_DATABASE: Ad[] = [
  {
    ...
    videoPoster: "https://grupoweprom.com/mktgdigital/Mercedes.mp4",
    videoPosterFallback: "",
...
function ExpandableText({ text }: ExpandableTextProps) {
  ...
  return (
    <div className="px-4 pb-4 pt-1 text-[15px] text-[#e4e6eb] text-left leading-[1.4] whitespace-pre-wrap transition-all duration-300">
      {displayText}
      {isLong && !isExpanded && (
        <button 
          onClick={() => setIsExpanded(true)}
          className="text-zinc-400 font-semibold hover:underline ml-1 focus:outline-none"
        >
          Ver más
        </button>
      )}
...
function AdHeader({ ad }: AdComponentProps) {
  return (
      ...
      <div className="flex items-center space-x-3 min-w-0">
        ...
        <div className="min-w-0 flex flex-col justify-center">
          <div className="flex items-center space-x-1">
            <span className="font-bold text-[15px] text-[#e4e6eb] hover:underline cursor-pointer truncate leading-tight">
              {ad.pageName}
            </span>
          ...
          <div className="flex flex-col mt-[2px]">
            <span className="text-[13px] text-[#b0b3b8] font-normal leading-none text-left flex items-center">
              ...
              <svg className="w-3 h-3 fill-current" viewBox="0 0 16 16"><path d="M8 0a8 8 0 100 16A8 8 0 008 0zm3.5 5a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM8 9.5a1.5 1.5 0 110 3 1.5 1.5 0 010-3zM4.5 5a1.5 1.5 0 110 3 1.5 1.5 0 010-3z"/></svg>
            </span>
          </div>
        </div>
      </div>
      <button className="text-[#b0b3b8] hover:bg-[#3a3b3c] p-2 rounded-full flex items-center justify-center transition flex-shrink-0 -mr-2">
...
function SocialActions() {
  return (
    <div className="px-4 py-1.5 border-t border-[#3e4042] flex justify-between text-[#b0b3b8] text-[14px] font-semibold bg-[#242526]">
      <button className="flex-1 flex items-center justify-center space-x-2 py-1.5 hover:bg-[#3a3b3c] rounded-md transition duration-200">
        <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
        </svg>
        <span>Me gusta</span>
      </button>
      <button className="flex-1 flex items-center justify-center space-x-2 py-1.5 mx-1 hover:bg-[#3a3b3c] rounded-md transition duration-200">
        <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
...
function SingleMediaAd({ ad }: AdComponentProps) {
  ...
  const togglePlay = () => {
    ...
    if (isPlaying) {
      ...
      videoRef.current.play()
        ...
        .catch((err) => console.log("Direct play failed:", err));
    }
  };
  ...
  return (
    <div className="w-[420px] max-w-[90vw] bg-[#242526] rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-[#3e4042] overflow-hidden font-sans flex-shrink-0 self-start transition-all duration-300 relative">
      <AdHeader ad={ad} />
      
      <ExpandableText text={ad.mainText} />

      <div className="relative bg-[#0e0f11] group overflow-hidden border-y border-[#3e4042] w-full flex items-center justify-center">
        {ad.videoUrl ? (
          <>
            <video 
              ref={videoRef}
              className="w-full h-auto max-h-[550px] object-contain cursor-pointer block"
              playsInline 
              loop 
              muted={isMuted}
              onClick={togglePlay}
              preload="metadata"
              poster={ad.videoPoster || undefined}
            >
            ...
            {!isPlaying && (
              <button 
                onClick={togglePlay}
                className="absolute w-14 h-14 rounded-full bg-black/60 hover:bg-black/80 flex items-center justify-center border border-white/20 transition transform hover:scale-105 z-10 backdrop-blur-sm shadow-lg"
              >
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent p-3 flex flex-col space-y-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
              ...
              <div className="flex items-center justify-between px-1">
                ...
                <button onClick={toggleMute} className="w-8 h-8 rounded-full bg-black/50 hover:bg-black/70 flex items-center justify-center text-white transition backdrop-blur-sm border border-white/10">
                  {isMuted ? (
                    <svg className="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24"><path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM4 9v6h4l5 5V4L8 9H4zM19 12c0 2.97-1.75-5.51-4.25 6.64l1.42 1.42C19.34 18.33 21 15.35 21 12s-1.66-6.33-4.83-8.06l-1.42 1.42C17.25 6.49 19 9.03 19 12z"/></svg>
                  ) : (
                    <svg className="w-[18px] h-[18px]" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9v6h4l5 5V4L8 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg>
                  )}
                </button>
              </div>
            </div>
...
function SingleImageAd({ ad }: AdComponentProps) {
  const isWhatsApp = ad.ctaDomain?.toLowerCase().includes('whatsapp') || ad.ctaBtnText?.toLowerCase().includes('mensaje');
  ...
  return (
    <div className="w-[420px] max-w-[90vw] bg-[#242526] rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-[#3e4042] overflow-hidden font-sans flex-shrink-0 self-start transition-all duration-300 relative">
      ...
      <div className="relative bg-[#0e0f11] overflow-hidden border-y border-[#3e4042] w-full flex items-center justify-center">
        {ad.imageUrl ? (
          <ImageWithFallback 
            src={ad.imageUrl}
...
function CarouselAd({ ad }: AdComponentProps) {
  const cards = ad.carouselCards || [];
  const [scrollIndex, setScrollIndex] = useState<number>(0);
  const visibleCards = 1.3; 
  const maxIndex = Math.max(0, cards.length - 1);

  const slideNext = () => setScrollIndex((prev) => (prev < maxIndex ? prev + 1 : prev));
  const slidePrev = () => setScrollIndex((prev) => (prev > 0 ? prev - 1 : 0));
  ...
  return (
    <div className="w-[420px] max-w-[90vw] bg-[#242526] rounded-xl shadow-[0_4px_12px_rgba(0,0,0,0.5)] border border-[#3e4042] overflow-hidden font-sans flex-shrink-0 self-start transition-all duration-300 relative">
      ...
      <div className="relative border-y border-[#3e4042] bg-[#18191a] pt-3 pb-4 px-3 overflow-hidden">
        <div 
        ...
        >
          {cards.map((card, idx) => (
            <div 
              key={idx} 
              className="w-[270px] flex-shrink-0 mr-3 bg-[#242526] rounded-xl border border-[#3e4042] overflow-hidden shadow-md flex flex-col group"
            >
              ...
              <div className="p-4 flex-grow flex flex-col justify-between bg-[#242526] border-t border-[#3e4042]">
                <div className="min-w-0 space-y-1 text-left">
                  <span className="block text-[10px] text-[#b0b3b8] font-bold tracking-wider uppercase truncate">
                    {card.domain}
                  </span>
                  <span className="block text-[15px] font-bold text-[#e4e6eb] truncate leading-snug group-hover:underline">
                    {card.title}
                  </span>
                </div>
        ...
        {scrollIndex > 0 && (
          <button 
            onClick={slidePrev}
            className="absolute left-4 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-[#242526] border border-[#3e4042] shadow-[0_2px_8px_rgba(0,0,0,0.5)] flex items-center justify-center text-[#e4e6eb] hover:bg-[#3a3b3c] transition-colors z-10"
          >
        ...
        {scrollIndex < maxIndex && (
          <button 
            onClick={slideNext}
            className="absolute right-4 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-[#242526] border border-[#3e4042] shadow-[0_2px_8px_rgba(0,0,0,0.5)] flex items-center justify-center text-[#e4e6eb] hover:bg-[#3a3b3c] transition-colors z-10"
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
...
export default function App() {
  ...
  const handleNextAd = () => {
    setActiveAdIndex((prev) => (prev < totalAds - 1 ? prev + 1 : 0));
  };

  const handlePrevAd = () => {
    setActiveAdIndex((prev) => (prev > 0 ? prev - 1 : totalAds - 1));
  };

  return (
    <div className="min-h-screen bg-[#0e0f11] text-zinc-100 flex flex-col xl:flex-row items-stretch overflow-x-hidden font-sans selection:bg-[#0866ff] selection:text-white">
      
      {/* Columna Izquierda: Texto Descriptivo Centrado */}
      <div className="w-full xl:w-1/3 p-8 lg:p-14 flex flex-col justify-center relative bg-gradient-to-br from-[#141516] to-[#0e0f11] z-20 xl:border-r xl:border-zinc-800/50 min-h-[40vh]">
        
        <div className="relative z-10 max-w-lg mx-auto flex flex-col items-center text-center">
          
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight text-white mb-4 leading-[1.1]" style={{ fontFamily: "'Astonpoliz', sans-serif" }}>
            Publicidad en <br/>
          ...
          <p className="text-lg md:text-xl text-zinc-400 leading-relaxed font-light mb-8">
            Llega a quien sí te va a comprar. Campañas publicitarias en Social Media optimizadas para generar resultados de manera <strong className="text-white font-medium">precisa y escalable</strong>.
          </p>

          {/* Isotipos de Redes Sociales */}
          <div className="flex items-center justify-center space-x-5 text-zinc-500">
            <svg className="w-6 h-6 hover:text-[#0866ff] transition-colors" fill="currentColor" viewBox="0 0 532.8 529.49" aria-label="Facebook">
              <path d="M308.11,529.49l-.06-186.13h61.67l12.23-76.9h-73.9v-54.17c.9-8.2,2.89-16.36,8-22.96,15.87-20.53,46.36-12.69,68.9-14.24v-65.91c-40.8-5.11-86.46-12.42-122.2,12.87-26.35,18.65-36.83,49.35-38.11,80.73-.86,21.01.68,42.62.02,63.69h-67.41v76.9h67.41l-.06,186.11C87.33,507.67-10.11,383.98.84,245.41,11.79,106.84,127.44-.02,266.44,0c139,.02,254.62,106.92,265.53,245.49,10.91,138.57-86.57,262.24-223.86,284Z"/>
            </svg>
            <svg className="w-6 h-6 hover:text-[#E1306C] transition-colors" fill="currentColor" viewBox="0 0 506.39 500" aria-label="Instagram">
            ...
            <svg className="w-6 h-6 hover:text-[#25D366] transition-colors" fill="currentColor" viewBox="0 0 535.06 538.99" aria-label="WhatsApp">
            ...
            <svg className="w-6 h-6 hover:text-[#106bff] transition-colors" fill="currentColor" viewBox="0 0 997.61 1037.17" aria-label="Messenger">
              <path d="M164.84,1037.17v-203.31C-159.21,547.76,25.68,42.95,448.51,2.38c338.84-32.51,630.68,273.05,528.34,611.12-77.6,256.35-358.42,392.33-612.98,326.77l-199.03,96.9ZM785.71,352.99l-212.69,116.47-122.09-121.31-244.7,260.46,219.99-121.35,119.67,128.64,239.82-262.91Z"/>
            </svg>
            <svg className="w-6 h-6 hover:text-white transition-colors" fill="currentColor" viewBox="0 0 509.05 587.81" aria-label="TikTok">
              <path d="M509.05,141v100c-49.69-.51-98.54-15.29-138.99-44l-.02,211.49c-6.18,119.68-117.9,204.7-235.41,172.43C10.31,546.78-40.73,394.49,36.64,291.09c40.07-53.55,106.55-82.16,173.41-73.09v102c-36.97-9.47-74.19,1.72-96.22,33.28-28.18,40.37-15.93,98.11,26.55,122.89,57.17,33.36,125.99-7.08,128.71-71.63l-.04-404.54h99.5l1.4,1.58c-1.05,7.52.1,16.94,1.39,24.63,8.81,52.29,50.61,95.57,100.94,110.06,4.33,1.25,16.53,4.74,20.26,4.74h16.5Z"/>
            </svg>
            <svg className="w-6 h-6 hover:text-white transition-colors" fill="currentColor" viewBox="0 0 576.08 523.59" aria-label="X">
              <path d="M541.52,0l-191.6,222.1,226.17,301.49h-176.02l-140.82-183.04-161.3,183.04H11.52l206.16-237.47L0,0h181.15l126.87,166.37,8.84-7.61L455.1,0h86.41ZM156.18,49.93h-52.49c2.65,7.2,8.85,14.05,13.57,20.35,96.61,128.79,195.64,260.7,295.72,386.61,3.89,4.89,7.66,10.77,12.67,14.22h44.81l1.36-2.6L156.18,49.93Z"/>
            </svg>
            <svg className="w-6 h-6 hover:text-[#0A66C2] transition-colors" fill="currentColor" viewBox="0 0 1280 1280" aria-label="LinkedIn">
            ...
            <svg className="w-6 h-6 hover:text-white transition-colors" fill="currentColor" viewBox="0 0 434.42 505.28" aria-label="Threads">
      ...
      {/* Columna Derecha: Visor de Anuncios del Carrusel rediseñado */}
      <div className="w-full xl:w-2/3 flex flex-col justify-center py-16 xl:py-0 relative bg-[#0e0f11] overflow-x-hidden overflow-y-visible min-h-[60vh]">
        
        {/* Efecto de foco centralizado */}
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-[#0866ff]/5 rounded-full blur-[100px] pointer-events-none z-0"></div>

        {/* Flecha Izquierda Flotante */}
        <button 
          onClick={handlePrevAd}
          className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-[#242526]/80 backdrop-blur-md hover:bg-[#3a3b3c] border border-zinc-700 shadow-2xl flex items-center justify-center text-white transition-all transform hover:scale-105 active:scale-95 z-30"
        ...
        <button 
          ...
          className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-[#242526]/80 backdrop-blur-md hover:bg-[#3a3b3c] border border-zinc-700 shadow-2xl flex items-center justify-center text-white transition-all transform hover:scale-105 active:scale-95 z-30"
        ...
        {/* Contenedor del Carrusel de Anuncios - Totalmente Flexible y Centrado dinámicamente */}
        <div className="relative w-full overflow-hidden flex flex-col justify-center items-center py-12 z-10 min-h-[850px]">
          ...
          <div 
            ...
            style={{
              transform: `translateX(calc(50% - 210px - ${activeAdIndex * 468}px))`,
              width: `${ADS_DATABASE.length * 468}px`,
            }}
          >
            {ADS_DATABASE.map((ad, idx) => {
              ...
              return (
                <div 
                  key={ad.id} 
                  className={`mx-6 flex-shrink-0 transition-all duration-700 ease-[cubic-bezier(0.25,1,0.5,1)] ${
                    isActive 
                      ? 'opacity-100 scale-100 z-20 shadow-[0_0_50px_rgba(8,102,255,0.15)]' 
                      : 'opacity-30 scale-[0.90] pointer-events-none blur-[0.5px]'
                  }`}
                  style={{ width: '420px' }}
                >
                  {/* CORRECCIÓN DE ENRUTAMIENTO: Evaluación explícita por tipo de render */}
                  {ad.type === 'carousel' ? (
                    <CarouselAd ad={ad} />
          ...
          {ADS_DATABASE.map((_, idx) => (
            <button
              ...
              className={`rounded-full transition-all duration-500 ease-out ${
                idx === activeAdIndex 
                  ? 'w-10 h-2 bg-[#0866ff] shadow-[0_0_10px_rgba(8,102,255,0.5)]' 
        ...
        {/* Gradientes laterales para enmarcar el carrusel */}
        <div className="absolute left-0 top-0 bottom-0 w-32 bg-gradient-to-r from-[#0e0f11] to-transparent z-20 pointer-events-none" />
        <div className="absolute right-0 top-0 bottom-0 w-32 bg-gradient-to-l from-[#0e0f11] to-transparent z-20 pointer-events-none" />
      </div>
    </div>
  );
}