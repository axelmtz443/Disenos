...
export default function AudiovisualSection() {
  ...
  // Manejadores nativos por si el usuario pausa desde los controles integrados del navegador
  ...
  return (
    <section id="audiovisual" className="relative min-h-screen py-24 bg-[#0a0a0a] overflow-hidden">
      {/* Glow orbs */}
      <div className="absolute top-1/4 left-1/4 w-100 h-100 rounded-full blur-[120px] pointer-events-none opacity-20" style={{ backgroundColor: '#599ddf' }} />
      <div className="absolute bottom-1/4 right-1/3 w-75 h-75 rounded-full blur-[120px] pointer-events-none opacity-15" style={{ backgroundColor: '#599ddf' }} />

      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        {/* Header */}
        <div className="mb-12">
          <span className="inline-block text-xs font-bold tracking-[0.25em] uppercase mb-3 opacity-90" style={{ color: '#599ddf' }}>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          {/* Contenedor Adaptable del Reproductor */}
          <div className="relative w-full bg-zinc-950 rounded-2xl overflow-hidden border border-zinc-800 shadow-2xl flex items-center justify-center max-h-[75vh]">
            ...
            {/* Capa informativa: Reaparece automáticamente en pausa o detención */}
            {!isPlaying && (
              <>
                ...
                >
                  <div
                    className="w-20 h-20 rounded-full backdrop-blur-md border flex items-center justify-center transition-all duration-300 group-hover/btn:scale-110 shadow-lg"
                    style={{
                      backgroundColor: 'rgba(89, 157, 223, 0.2)',
                      borderColor: 'rgba(89, 157, 223, 0.4)'
                    }}
                  >
                ...
                {/* Métricas y Datos superpuestos */}
                <div className="absolute bottom-0 left-0 right-0 p-6 text-white pointer-events-none">
                  <span
                    className="inline-block px-3 py-1 rounded-full text-xs font-semibold mb-2"
                    style={{ backgroundColor: 'rgba(89, 157, 223, 0.2)', color: '#599ddf' }}
                  >
                    {currentProject.client}
                  </span>
                  <h3 className="text-xl md:text-2xl font-bold mb-3" style={{ fontFamily: 'Astonpoliz, sans-serif' }}>
                    {currentProject.title}
                  </h3>
                  <div className="flex gap-6 text-sm text-zinc-300" style={{ fontFamily: 'Montserrat, sans-serif' }}>
                    <div className="flex items-center gap-2">
                      <svg className="w-4 h-4" style={{ color: '#599ddf' }} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                    ...
                    <div className="flex items-center gap-2">
                      <svg className="w-4 h-4" style={{ color: '#80b67d' }} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-zinc-500 mb-1" style={{ fontFamily: 'Montserrat, sans-serif' }}>{project.client}</p>
                    <p className="text-sm font-semibold text-white truncate" style={{ fontFamily: 'Astonpoliz, sans-serif' }}>{project.title}</p>
                  </div>
                  <div
                    className="shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-colors"
                    style={{ backgroundColor: activeProject === idx ? '#599ddf' : '#27272a' }}
                  >
